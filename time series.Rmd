...
#transpose t()
starbucks.metrics <- t(read.csv("starbucks_metrics2.csv", header=FALSE))
...
chipotle.sales <- read.csv("starbucks_sales.csv")
...
starbucks_sale <- chipotle.sales2$Sales


# Creating time series
starbucks_ts <- ts(starbucks_sale, start = c(2015, 1), end=c(2017,3), frequency=4)
plot(starbucks_ts, main="Time Series for Starbucks Revenues")

# Exponential Models
fit <- HoltWinters(starbucks_ts, beta=FALSE, gamma=FALSE)
forecast(fit, 3)
plot(forecast(fit,3))


## Automated Forecasting
fit.1 <- ets(starbucks_ts)
fit.2 <- auto.arima(starbucks_ts)


# ARIMA forecasting
acf(starbucks_ts, main="ACF")
pacf(starbucks_ts, main="PACF")

fit.3 <- arima(starbucks_ts, order = c(1,0,1))

accuracy(fit.3)
plot(forecast(fit.3, 1))
```