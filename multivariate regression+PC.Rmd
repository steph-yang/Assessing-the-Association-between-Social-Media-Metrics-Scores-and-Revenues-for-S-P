---
title: "Multivariate Regression and Monte Carlo"
...
# plot
plot(1:8, rstudent(fit.star))
lines(1:8, rstudent(fit.star), col=2)
abline(h=0, lty=3)
```
...
```{r}
library(arm)
sim.1 <- sim(fit.star,1000)
meanPC <- mean(PC1_data)
Y.star <- sim.1@coef[,1]+sim.1@coef[,2]*meanPC+sim.1@sigma
hist(Y.star, seq(2000,3500,50))
````
