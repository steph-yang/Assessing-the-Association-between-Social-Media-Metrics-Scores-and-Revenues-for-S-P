...
prcomp(starbucks.metrics3)

PC1 <- output.chi$rotation[,1]
PC2 <- output.chi$rotation[,2]
PC3 <- output.chi$rotation[,3]
PC4 <- output.chi$rotation[,4]
PC5 <- output.chi$rotation[,5]

glm_sales <- c(0,1,1,1,0,1,1,1)


length(PC1)
length(starbucks.metrics3)


PC1_data <- PC1%*%t(starbucks.metrics3)
new_sales <- starbucks.sales2$Sales[1:8]

PC1_data <- as.numeric(PC1_data)

fit.star<- lm(new_sales~PC1_data)

fit.star.2 <- glm(glm_sales~PC1_data, family = binomial(link = "logit"))
summary(fit.star.2)
```
